            ...
            <div className=" flex flex-wrap justify-evenly w-full min-h-[60vh] py-8 border border-gray-100 rounded">

                {
                    trendData && trendData.map(coin => 
                        <TrendingCoin key={data.id} data={coin.item} />
                        )
                }
                            <button className=" w-8 ml-4 hover:rotate-180 transition-all  ease-in-out absolute -top-10 right-2" onClick={resetTrendingResult}>
