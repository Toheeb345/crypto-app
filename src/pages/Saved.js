...
const SaveBtn = ({data}) => {
    ...
    return(
        <button className=" outline-0 border-0 bg-none cursor-pointer" onClick={(e) => handleClick(e)}>
            <svg width="30" className={` w-[1.5rem] ml-1.5  hover:fill-cyan ${allCoins.includes(data.id) ? "fill-cyan" : "fill-gray-100"}`} height="30" viewBox="0 0 30 30" fill="cyan" xmlns="http://www.w3.org/2000/svg">
...
<tbody>
{
    ...
    <h1 className=" min-h-[60vh] flex justify-center items-center text-xl animate-bounce capitalize first-letter:text-cyan">No saved coins!</h1>

}
    <button className=" w-8 ml-4 hover:rotate-[360deg] transition-all  ease-in-out absolute -top-10 right-2" onClick={resetSavedResult}>
